...
use super::util::{enumerate, ptrdistance, third};
use super::{Bucket, Entries, HashValue};
...
///     *letters.entry(ch).unwrap().or_insert(0).unwrap() += 1;
/// }
///
...
/// ```
pub struct IndexMap<K, V, S = RandomState, A: Alloc = Global> {
    core: OrderMapCore<K, V, A>,
    hash_builder: S,
}
...
{
    fn clone(&self) -> Self {
        self.try_clone().expect("Cloning an index map failed")
    }
}
...
where
    ...
    A: Alloc + Clone,
{
    /// Tries to clone the index map.
    pub fn try_clone(&self) -> Result<Self, Error> {
        let core = self.core.try_clone()?;
        let hash_builder = self.hash_builder.clone();
        Ok(IndexMap { core, hash_builder })
    }
}
...
impl<K, V, A: Alloc> OrderMapCore<K, V, A> {
    fn take_entries(self) -> Vec<Bucket<K, V>, A> {
        self.entries
    }
}
...
impl<K, V, S, A: Alloc> fmt::Debug for IndexMap<K, V, S, A>
where
    K: fmt::Debug + Hash + Eq,
    V: fmt::Debug,
    S: BuildHasher,
...
impl<K, V> IndexMap<K, V> {
    ...
    pub fn new() -> Self {
        Self::with_capacity(0).expect("No allocation expected")
    }
...
impl<K, V, A: Alloc + Clone> IndexMap<K, V, RandomState, A> {
    /// Create a new map. (Does not allocate.)
    pub fn new_in(alloc: A) -> Self {
        Self::with_capacity_in(0, alloc).expect("No allocation expected")
    }
    ...
    ///
    /// Computes in **O(n)** time.
    pub fn with_capacity_in(n: usize, alloc: A) -> Result<Self, Error> {
...
impl<K, V, S> IndexMap<K, V, S> {
    ...
    ///
    /// Computes in **O(n)** time.
    pub fn with_capacity_and_hasher(n: usize, hash_builder: S) -> Result<Self, Error>
    where
        S: BuildHasher,
    {
        Self::with_capacity_and_hasher_in(n, hash_builder, <_>::default())
    }
    ...
    where
        S: BuildHasher,
    {
        Self::with_capacity_and_hasher_in(0, hash_builder, <_>::default())
    }
}

impl<K, V, S, A: Alloc> IndexMap<K, V, S, A> {
    /// Create a new map with capacity for `n` key-value pairs. (Does not
    /// allocate if `n` is zero.)
    ///
    /// Computes in **O(n)** time.
        ...
        A: Clone,
    {
        if n == 0 {
            ...
            let indices =
                Vec::repeat_in(Pos::none(), raw_cap, alloc.clone())?.try_into_boxed_slice()?;
            Ok(IndexMap {
                core: OrderMapCore {
    ...
    /// Create a new map with `hash_builder`
    pub fn with_hasher_in(hash_builder: S, alloc: A) -> Result<Self, Error>
    where
        S: BuildHasher,
        A: Clone,
    {
...
/// insert one.
pub enum Entry<'a, K: 'a, V: 'a, A: Alloc> {
...
impl<'a, K, V, A: Alloc> Entry<'a, K, V, A> {
    ...
    /// Computes in **O(1)** time (amortized average).
    pub fn or_insert_with<F>(self, call: F) -> Result<&'a mut V, Error>
    where
        F: FnOnce() -> V,
    {
        match self {
...
impl<'a, K: 'a + fmt::Debug, V: 'a + fmt::Debug, A: Alloc + Clone> fmt::Debug
    for Entry<'a, K, V, A>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
...
pub struct OccupiedEntry<'a, K: 'a, V: 'a, A: Alloc> {
    map: &'a mut OrderMapCore<K, V, A>,
    key: K,
    probe: usize,
    index: usize,
}

impl<'a, K, V, A: Alloc> OccupiedEntry<'a, K, V, A> {
    pub fn key(&self) -> &K {
        &self.key
    }
    pub fn get(&self) -> &V {
...
///
/// [`Entry`]: enum.Entry.html
pub struct VacantEntry<'a, K: 'a, V: 'a, A: Alloc> {
    map: &'a mut OrderMapCore<K, V, A>,
    key: K,
    hash: HashValue,
    probe: usize,
...
impl<'a, K, V, A: Alloc> VacantEntry<'a, K, V, A> {
    pub fn key(&self) -> &K {
        &self.key
    }
    ...
    fn insert_impl<Sz>(self, value: V) -> Result<&'a mut V, Error>
    where
        Sz: Size,
    {
...
impl<'a, K: 'a + fmt::Debug, V: 'a, A: Alloc> fmt::Debug for VacantEntry<'a, K, V, A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    S: BuildHasher,
{
    ...
    fn entry_phase_1<Sz>(&mut self, key: K) -> Entry<K, V, A>
    where
        Sz: Size,
    {
    ...
    fn insert_phase_1<Sz>(&mut self, key: K, value: V) -> Result<Inserted<V>, Error>
    where
        Sz: Size,
    {
        let hash = hash_elem_using(&self.hash_builder, &key);
    ...
    fn double_capacity<Sz>(&mut self) -> Result<(), Error>
    where
        Sz: Size,
        A: Clone,
    {
    ...
    {
        self.reserve_one()?;
        Ok(if self.size_class_is_64bit() {
    ...
    /// See also [`entry`](#method.entry) if you you want to insert *or* modify
    /// or if you need to get the index of the corresponding key-value pair.
    pub fn insert_full(&mut self, key: K, value: V) -> Result<(usize, Option<V>), Error>
    where
        A: Clone,
    {
    ...
    ///
    /// Computes in **O(1)** time (amortized average).
    pub fn entry(&mut self, key: K) -> Result<Entry<K, V, A>, Error>
    where
        A: Clone,
    {
        self.reserve_one()?;
        Ok(dispatch_32_vs_64!(self.entry_phase_1(key)))
    }
    ...
    pub fn drain(&mut self, range: RangeFull) -> Drain<K, V, A> {
...
impl<K, V, S, A: Alloc + Clone> IndexMap<K, V, S, A> {
...
impl<K, V, A: Alloc> OrderMapCore<K, V, A> {
    ...
    fn first_allocation(&mut self) -> Result<(), Error>
    where
        A: Clone,
    {
        ...
        self.indices =
            Vec::repeat_in(Pos::none(), raw_cap, allocator.clone())?.try_into_boxed_slice()?;
        ...
        A: Clone,
    {
        ...
        self.entries.try_reserve_exact(more)?;
        Ok(())
    }
        ...
        if let Some((i, hash_proxy)) = pos.resolve::<SzOld>() {
            ...
            probe_loop!(probe < indices.len(), {
                if indices[probe].is_none() {
                    ...
                    indices[probe] = Pos::with_hash::<SzNew>(i, entry_hash);
                    return;
                }
            });
        }
    ...
    // FIXME: reduce duplication (compare with insert)
    fn entry_phase_1<Sz>(&mut self, hash: HashValue, key: K) -> Entry<K, V, A>
    where
        Sz: Size,
        K: Eq,
    {
    ...
    // When we insert they key, it might be that we need to continue displacing
    // entries (robin hood hashing), in which case Inserted::RobinHood is returned
    fn insert_phase_1<Sz>(
        &mut self,
        hash: HashValue,
        key: K,
        value: V,
    ) -> Result<Inserted<V>, Error>
    where
        Sz: Size,
        K: Eq,
    {
        ...
        probe_loop!(probe < indices.len(), {
            if let Some((i, hash_proxy)) = indices[probe].resolve::<Sz>() {
                ...
                if probe_distance(self.mask, entry_hash.into_hash(), probe) > 0 {
                    indices[last_probe] = indices[probe];
                    indices[probe] = Pos::none();
                } else {
                    break;
                }
            } else {
        ...
        Sz: Size,
    {
        ...
        for i in 0..len {
            ...
            if !will_keep {
                ...
                indices[probe].set_pos::<Sz>(i - n_deleted);
...
where
    ...
    A: Alloc,
{
    pub(crate) iter: crate::vec::Drain<'a, Bucket<K, V>, A>,
}

impl<'a, K, V, A: Alloc> Iterator for Drain<'a, K, V, A> {
    type Item = (K, V);

    iterator_methods!(Bucket::key_value);
}

impl<'a, K, V, A: Alloc> DoubleEndedIterator for Drain<'a, K, V, A> {
    double_ended_iterator_methods!(Bucket::key_value);
}

impl<'a, K, V, S, A: Alloc> IntoIterator for &'a IndexMap<K, V, S, A>
where
    K: Hash + Eq,
    S: BuildHasher,
{
...
impl<'a, K, V, S, A: Alloc> IntoIterator for &'a mut IndexMap<K, V, S, A>
where
    K: Hash + Eq,
    S: BuildHasher,
{
...
impl<K, V, S, A: Alloc> IntoIterator for IndexMap<K, V, S, A>
where
    K: Hash + Eq,
    S: BuildHasher,
{
...
impl<'a, K, V, Q: ?Sized, S, A: Alloc> Index<&'a Q> for IndexMap<K, V, S, A>
where
    Q: Hash + Equivalent<K>,
    K: Hash + Eq,
    S: BuildHasher,
...
impl<'a, K, V, Q: ?Sized, S, A: Alloc> IndexMut<&'a Q> for IndexMap<K, V, S, A>
where
    Q: Hash + Equivalent<K>,
    K: Hash + Eq,
    S: BuildHasher,
    ...
    S: BuildHasher + Default,
{
    ...
    fn from_iter<I: IntoIterator<Item = (K, V)>>(iterable: I) -> Self {
        ...
        let mut map =
            Self::with_capacity_and_hasher(low, <_>::default()).expect("Map creation failed");
        map.extend(iter);
        map
    }
}

impl<K, V, S, A: Alloc> Extend<(K, V)> for IndexMap<K, V, S, A>
where
    K: Hash + Eq,
    S: BuildHasher,
    A: Clone,
{
    ...
    fn extend<I: IntoIterator<Item = (K, V)>>(&mut self, iterable: I) {
        for (k, v) in iterable {
            self.insert(k, v).expect("Can't extend a map");
        }
    }
}

impl<'a, K, V, S, A: Alloc> Extend<(&'a K, &'a V)> for IndexMap<K, V, S, A>
where
    K: Hash + Eq + Copy,
    V: Copy,
    S: BuildHasher,
...
impl<K, V, S, A: Alloc> Eq for IndexMap<K, V, S, A>
where
    K: Eq + Hash,
    V: Eq,
    S: BuildHasher,
...
mod tests {
    use super::*;
    use crate::indexmap;
    use crate::indexmap::util::enumerate;
    ...
    fn insert() {
        ...
        let mut map = IndexMap::with_capacity(insert.len()).unwrap();

        for (i, &elt) in enumerate(&insert) {
            assert_eq!(map.len(), i);
            map.insert(elt, elt).unwrap();
    ...
    fn insert_full() {
        ...
        for (i, &elt) in enumerate(&insert) {
            assert_eq!(map.len(), i);
            let (index, existing) = map.insert_full(elt, elt).unwrap();
    ...
    #[test]
    fn insert_2() {
        let mut map = IndexMap::with_capacity(16).unwrap();
    ...
    fn grow() {
        ...
        println!("{:?}", map);
        for &elt in &insert {
            map.insert(elt * 10, elt).unwrap();
        }
        for &elt in &insert {
            map.insert(elt * 100, elt).unwrap();
        }
        for (i, &elt) in insert.iter().cycle().enumerate().take(100) {
            map.insert(elt * 100 + i as i32, elt).unwrap();
        }
        println!("{:?}", map);
        for &elt in &not_present {
            assert!(map.get(&elt).is_none());
    ...
    #[test]
    fn remove_to_empty() {
        let mut map = indexmap! { 0 => 0, 4 => 4, 5 => 5 }.unwrap();
    ...
    fn partial_eq_and_eq() {
        ...
        map_a.insert(1, "1").unwrap();
        map_a.insert(2, "2").unwrap();
    ...
    fn entry() {
        ...
        {
            let e = map.entry(3).unwrap();
            assert_eq!(e.index(), 2);
            let e = e.or_insert("3").unwrap();
            assert_eq!(e, &"3");
        }

        let e = map.entry(2).unwrap();
        assert_eq!(e.index(), 1);
        assert_eq!(e.key(), &2);
        match e {
        ...
        assert_eq!(e.or_insert("4").unwrap(), &"2");
    }
    ...
    fn entry_and_modify() {
        ...
        map.insert(1, "1").unwrap();
        map.entry(1).unwrap().and_modify(|x| *x = "2");
        assert_eq!(Some(&"2"), map.get(&1));

        map.entry(2).unwrap().and_modify(|x| *x = "doesn't exist");
    ...
    fn entry_or_default() {
        ...
        map.insert(1, TestEnum::NonDefaultValue).unwrap();
        assert_eq!(
            &mut TestEnum::NonDefaultValue,
            map.entry(1).unwrap().or_default().unwrap()
        );

        assert_eq!(
            &mut TestEnum::DefaultValue,
            map.entry(2).unwrap().or_default().unwrap()
        );
    }
